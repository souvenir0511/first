 ...
 */
@Slf4j
public class Spider2MongoToMysql {

    static MongoClient fromMongoClient;
    static MongoClient fromMongoClient2;
    static MongoClient toMongoClient;
    static MongoDatabase fromMongoDatabase;
    static MongoDatabase fromMongoDatabase2;
    static MongoDatabase toMongoDatabase;
    static MongoCollection<Document> fromMongoCollection;
    static MongoCollection<Document> fromMongoCollection2;
    static MongoCollection<Document> toMongoCollection;


    static String formMongoHost = "192.168.10.130";
    static String formMongoHost2 = "192.168.10.130";
    static String toMongoHost = "192.168.10.150";
    static int mongoPort = 27017;
    static int mongoPort2 = 27017;
    static String mongoUser = "";
    static String mongoPwd = "";
    static String fromMongoDatabaseName = "QCC_DATA";
    static String fromMongoDatabaseName2 = "ZLJDATA";
    static String fromMongoCollectionName = "qcc_basicInfo";
    static String fromMongoCollectionName2 = "zlj_basic_info";
    static String toMongoDatabaseName = "zgg_data";
    static String toMongoCollectionName = "20211029_enterprise";

    static String driver = "com.mysql.cj.jdbc.Driver";
    static String url="jdbc:mysql://192.168.10.150:3306/spider_qcc_zlj?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&autoReconnect=true&failOverReadOnly=false&rewriteBatchedStatements=true";
    ...
    static String url2="jdbc:mysql://192.168.10.150:3306/zgg_spider?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&autoReconnect=true&failOverReadOnly=false&rewriteBatchedStatements=true";
    ...
    public static void init() {
        ...
        fromMongoClient2 = new MongoClient(formMongoHost2, mongoPort2);
        fromMongoDatabase2 = fromMongoClient2.getDatabase(fromMongoDatabaseName2);
        fromMongoCollection2 = fromMongoDatabase2.getCollection(fromMongoCollectionName2);
    ...
    public static void fixEnterpriseData() throws SQLException {
        ...
        List<Entity> entityList = SqlExecutor.query(connection, "select enterprise_name,type,years from growth_enterprise", new EntityListHandler());
        ...
        while (++times < batchSize){
            try {
                ...
                Set<String> applicantSet = documents.stream().map(item -> item.getString("company")).collect(Collectors.toSet());
                //过滤企业
                List<Entity> entities = entityList.stream().filter(item -> applicantSet.contains(item.getStr("enterprise_name"))).collect(Collectors.toList());
                //查询专利局专利
                List<Document> patentBaseInfoList = new ArrayList<>();
                for (Document document : fromMongoCollection2.find(new Document("applicant", new Document("$in", applicantSet)))) {
                    patentBaseInfoList.add(document);
                }
                log.info( "查询专利局mango数据" + patentBaseInfoList.size() + "条,耗时" + (System.currentTimeMillis()-start) + "ms;" );
                Map<String, List<Document>> groupingByApplicantMap = patentBaseInfoList.stream().collect(Collectors.groupingBy(item -> item.getString("applicant")));

                start = System.currentTimeMillis();
                List<Document> list = Lists.newArrayList();
                List<EnterpriseExpand> enterpriseExpandList = new ArrayList<>();
...
//                AtomicInteger entPatentCount = new AtomicInteger();
                documents.forEach(item -> {
                    List<Document> patentBaseInfos = new ArrayList<>();
                    if (ObjectUtils.isNotEmpty(groupingByApplicantMap) && CollectionUtils.isNotEmpty(groupingByApplicantMap.get(item.getString("company")))) {

                        patentBaseInfos = groupingByApplicantMap.get(item.getString("company"));

                    }
                    //企业基本信息mysql
                    EnterpriseExpand enterpriseExpand = DocumentToBeanUtil.spiderMongoToMysqlEnt(item, entities, patentBaseInfos);
                    enterpriseExpandList.add(enterpriseExpand);
//                    //企业工商数据
...
//                    //插入MYSQL
                    enterpriseExpandBatch(enterpriseExpandList);
...
//                    entFormerNameBatch(entFormerNameList);
     ...
     * @param
     */
    public static void enterpriseExpandBatch(List<EnterpriseExpand> enterpriseExpandList){

        //数据库插入
        int amount;
        if (CollectionUtil.isNotEmpty(enterpriseExpandList)) {
            amount = enterpriseExpandList.size();
            int pageSize = 5000;
...
//                executorService.submit(() -> {
                    List<EnterpriseExpand> enterpriseExpands = enterpriseExpandList.subList(fromIndex, finalToIndex);
                    Enterprise2Processor(enterpriseExpands);
...
//                });
    ...
    public static void Enterprise2Processor(List<EnterpriseExpand> enterpriseExpandList) {
        long start = System.currentTimeMillis();
        Connection conn = getPool();
        //字段
        String field = "ent_id,ent_name,old_ent_names,fa_ren,reg_money,ent_status,reg_date,credit_code,ent_type,industry,experience_scope,introduce,gj_gao_xin,zgc_gao_xin,jin_zhong_zi,deng_ling_yang,du_jiao_shou,jin_zhuan_te_Xin,brand_count,patent_count,software_copyright_count,copyright_count,certificate_count,inv_event_count,finance_count,judgment_document_count,invent_patent_count,practical_patent_count,exterior_patent_count,qcc_invent_patent_count,qcc_practical_patent_count,qcc_exterior_patent_count,insured_number,address,export_import_enterprise_code,finance_rounds,brand_agency,patent_agency,patent_office_count,tags,province,city,area,high_tech_years";
        //占位符
        String placeholder = "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?";
        //sql
        String sql = "insert into `enterprise`(" + field + ")values(" + placeholder + ")";
        List<Object[]> objectsList = new ArrayList<>();
        enterpriseExpandList.forEach(item -> {
            //值
            Object[] objects = {
                    ...
                    .getCopyrightCount(),item.getCertificateCount(),item.getInvEventCount(),item.getFinanceCount(),item.getJudgmentDocumentCount(),item.getInventPatentCount(),item.getPracticalPatentCount(),item.getExteriorPatentCount(),item.getQccInventPatentCount(),item.getQccPracticalPatentCount(),item.getQccExteriorPatentCount(),item.getInsuredNumber(),item.getAddress(),item.getExportImportEnterpriseCode(),item.getFinanceRounds(),item.getBrandAgency(),item.getPatentAgency(),item.getPatentOfficeCount(),item.getTags(),item.getProvince(),item.getCity(),item.getArea(),item.getHighTechYears()
            };
        ...
        log.info("enterpriseExpand，"+enterpriseExpandList.size() + "条耗时" + (System.currentTimeMillis() - start) + "ms;");
    }
