 ...
 */
@Slf4j
public class MongoForFindToMysql {
...
//        fromMongoClient = new MongoClient(formMongoHost, mongoPort);
    ...
    public static void fixEnterpriseData() {
        ...
        System.out.println("请输入申请人名称（多个用英文逗号连接）:");
        String applicant = scanner.nextLine();  //读取字符串型输入
        ...
        try {
            ...
            if (StringUtils.isNotBlank(applicant)) {
                List<String> applicantList = Arrays.asList(applicant.split(","));
                documentFindIterable = fromMongoCollection.find(new Document("applicant", new Document("$in", applicantList)));
            }else {

                documentFindIterable = fromMongoCollection.find();
            }
...
//                    toMongoCollection.insertMany(patentList);
                ...
                log.info("size:{};;耗时:{}ms;", patentList.size(), System.currentTimeMillis() - start);
            }catch (Exception e){
