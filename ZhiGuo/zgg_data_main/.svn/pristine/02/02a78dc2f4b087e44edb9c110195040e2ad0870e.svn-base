 ...
 */
@Slf4j
public class UniversityToMysql {
    ...
    static String url="jdbc:mysql://192.168.10.154:3306/zgg_business?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&autoReconnect=true&failOverReadOnly=false&rewriteBatchedStatements=true";
...
//                    list.add(doc);
                    University university = DocumentToBeanUtil.toMysqlUniversity(document);
...
//                    toMongoCollection.insertMany(list);
                    ...
                    entEnterpriseBatch(universityList);
...
//                    financeBatch(financeList);
                    //jgudgmentDocumentBatch(judgmentDocumentList);
...
//                    entFormerNameBatch(entFormerNameList);
     ...
     * @param
     */
    public static void entEnterpriseBatch(List<University> universityList){

        //数据库插入
        int amount;
        if (CollectionUtil.isNotEmpty(universityList)) {
            amount = universityList.size();
            int pageSize = 10000;
...
//                executorService.submit(() -> {
                    List<University> universities = universityList.subList(fromIndex, finalToIndex);
                    universityProcessor(universities);
//                    latch.countDown();
//                });
            }
        }
    ...
    public static void universityProcessor(List<University> universityList) {
        long start = System.currentTimeMillis();
        Connection conn = getPool();
        //字段
        String field = "id,name,old_names,legal_person,registered_capital,money_unit,address,phone,website,email,introduce,tags,status,registration_date,credit_code,experience_scope,brand_count,patent_count,software_copyright_count,copyright_count,certificate_count,invest_count,finance_count,judgment_count,order_index,cooperation_enterprise_count,inventor_count";
        //占位符
        String placeholder = "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?";
        //sql
        String sql = "insert into `university_copy1`(" + field + ")values(" + placeholder + ")";
        List<Object[]> objectsList = new ArrayList<>();
        universityList.forEach(item -> {

            //值
            Object[] objects = {
                    item.getId(),item.getName(),item.getOldNames(),item.getLegalPerson(),item.getRegisteredCapital(),item.getMoneyUnit(),item.getAddress(),item.getPhone(),item.getWebsite(),item.getEmail(),item.getIntroduce(),item.getTags(),item.getStatus(),item.getRegistrationDate(),item.getCreditCode(),item.getExperienceScope(),item.getBrandCount(),item.getPatentCount(),item.getSoftwareCopyrightCount(),item.getCopyrightCount(),item.getCertificateCount(),item
                    .getInvestCount(),item.getFinanceCount(),item.getJudgmentCount(),item.getOrderIndex(),item.getCooperationEnterpriseCount(),item.getInventorCount()
            };
        ...
        log.info("universityProcessor耗时{}ms",(System.currentTimeMillis() - start));
    }
    ...
    public static void entCertificateProcessor(List<EntCertificate> entCertificateList) {
        ...
        Connection conn = getPool();
        //字段
        String field = "id,ent_id,certificate_type,certificate_name,certificate_no,issue_date,end_date";
        //占位符
        String placeholder = "?,?,?,?,?,?,?";
        //sql
        ...
        entCertificateList.forEach(item -> {
            ...
            //值
            Object[] objects = {
                    item.getId(),item.getEntId(),item.getCertificateType(),item.getCertificateName(),item.getCertificateCode(),item.getCreateDate(),item.getEndDate()
            };
            objectsList.add(objects);
        });
    ...
    public static void judgmentProcessor(List<Judgment> judgmentList) {
        ...
        log.info("judgmentProcessor任务结束，"+judgmentList.size() + "条耗时" + (System.currentTimeMillis() - start) / 1000 + "s;");
    }
    ...
    public static void changeRecordProcessor(List<ChangeRecord> changeRecordList) {
        ...
        log.info("changeRecordProcessor任务结束，"+changeRecordList.size() + "条耗时" + (System.currentTimeMillis() - start) / 1000 + "s;");
    }
    ...
    public static void shareHolderProcessor(List<ShareHolder> shareHolderList) {
        ...
        log.info("shareHolderProcessor任务结束，"+shareHolderList.size() + "条耗时" + (System.currentTimeMillis() - start) / 1000 + "s;");
    }
    ...
    public static void entFormerNameProcessor(List<EntFormerName> entFormerNameList) {
        ...
        log.info("entFormerNameProcessor任务结束，"+entFormerNameList.size() + "条耗时" + (System.currentTimeMillis() - start) / 1000 + "s;");
    }
}
